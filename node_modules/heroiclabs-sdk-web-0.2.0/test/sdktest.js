...
var apikey = config['apikey']
    ...
    cloudcodeModule = config['cloudcode_module'],
    cloudcodeFunction = config['cloudcode_function'],
...
describe('', function() {
  ...
  it('Get Leaderboard Rank', function() {
    ...
    request.withScoretags().limit(11).autoOffset().filterBySocialIds('web_socialid, web_missing_socialId');
    return client.execute(request).then(function (r) {
  ...
  it('Get Queue Status', function() {
    var request = new w.MatchQueueStatusRequest(session);
    return client.execute(request).then(function (r) {
      expect(r.status).to.be.equal(204);
    });
  });

  it('Matchmaking Dequeue', function() {
    var request = new w.MatchDequeueRequest(session);
    return client.execute(request).then(function (r) {
      expect(r.status).to.be.equal(204);
    });
  });
  ...
  it('Execute Cloud Code function', function() {
    var request = new w.ExecuteCloudCodeFunctionRequest(cloudcodeModule, cloudcodeFunction);
